/**
 * Pure TypeScript RFC 1321 MD5 & RFC 2104 HMAC-MD5 Implementation
...
/**
 * Computes MD5 Hash of a UTF-8 string
 */
...
function coreHmacMd5(key: string, data: string): number[] {
  ...
  const hash1 = binlMD5(ipad.concat(str2binl(data)), 512 + data.length * 8)
  return binlMD5(opad.concat(hash1), 512 + 128)
}

/**
 * Computes HMAC-MD5 of a UTF-8 string using key
 */
export function hmacMd5(key: string, data: string): string {
  const k = unescape(encodeURIComponent(key))
  const d = unescape(encodeURIComponent(data))
  return binl2hex(coreHmacMd5(k, d))
}
